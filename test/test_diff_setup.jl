...
@testset "Differentiation: Setup-layer and probe helpers" begin
    ...
    #    Internally calls _akima_interpolation(matrix, Blast.z_lin, x)
    #    where x = bg.z_of_χ.(new_χs) are fixed Float64 query points.
    #    The rrule for _akima_interpolation covers ForwardDiff and Mooncake.
    #    Zygote is skipped: reverse-mode fails with `UndefVarError: j` when
    #    tracing through reshape(…, length(Blast.R), …) on generated code.
    #    Mooncake is skipped: `_akima_interpolation_matrix_fused_pullback`
    #    (called via @from_chainrules) returns a Vector tangent for the
    #    LinRange `t` argument; Mooncake has no `increment_and_get_rdata!`
    #    for (NoFData, RData{LinRange fields}, Vector) and throws.

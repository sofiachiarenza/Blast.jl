...
function compute_kernel!(Component::CosmicShear, grid::CosmologicalGrid, bg::BackgroundQuantities, cosmo::AbstractCosmology)
    ...
    for b in 1:n_bins
        ...
        for z_idx in 1:length(grid.z_range)
            integrand(x) = nz_func(x) * (1.0 - bg.χz_array[z_idx] / compute_χ(x, cosmo))
            ...
            kernel[b, z_idx] = prefac * bg.χz_array[z_idx] * (1.0 + grid.z_range[z_idx]) * int / nz_norm
        end
    end
    Component.Kernel = kernel
end
...
function compute_kernel!(Component::CMBLensing, grid::CosmologicalGrid, bg::BackgroundQuantities, cosmo::AbstractCosmology)
    ...
    kernel = @. prefac * bg.χz_array * (1.0 + grid.z_range) * (1 - bg.χz_array / χ_CMB)
...
function compute_kernel!(Component::MagnificationBias, grid::CosmologicalGrid, bg::BackgroundQuantities, cosmo::AbstractCosmology)
    ...
    for b in 1:n_bins
        ...
        for z_idx in 1:length(grid.z_range)
            integrand(x) = nz_func(x) * (1.0 - bg.χz_array[z_idx] / compute_χ(x, cosmo)) * (5 .* s_z(x) .- 2)
            z_low = grid.z_range[z_idx]
...
function compute_kernel_test!(Component::MagnificationBias, grid::CosmologicalGrid, bg::BackgroundQuantities, cosmo::AbstractCosmology)
